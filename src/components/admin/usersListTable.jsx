...
const createData=(name, email, phone,active, role, verified,createdAt,updatedAt,dateOfBirth)=> {
  return {
    name,
    email,
    phone,
...
const createRows =(setRows,data)=>{
 setRows( data.map((item) =>
  createData(
    `${item.fullName.firstName} ${item.fullName.lastName}`,
    item.email,
    item.phone,
    item.active ? 'Actived' : 'Not Actived',
    item.role,
    item.verified ? 'Verified' : 'Not Verified',
    convertDateTime(item.createdAt),
    convertDateTime(item.updatedAt),
    item.dateOfBirth,
  )
))
} 
